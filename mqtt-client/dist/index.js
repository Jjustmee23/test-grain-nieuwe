...
const official_uc300_decoder_1 = require("./official-uc300-decoder");
const uc300_command_sender_1 = require("./uc300-command-sender");
...
async function storeUC300OfficialData(deviceId, topic, payload, processingTime) {
    try {
        const data = {
            device_id: deviceId,
            topic,
            ...
            decoded_data: JSON.stringify(official_uc300_decoder_1.OfficialUC300Decoder.getReadableStatus(payload)),
            // Performance
...
function handleMessage(topic, payload) {
    ...
    try {
        ...
        const decodedPayload = official_uc300_decoder_1.OfficialUC300Decoder.decodePayload(payload);
        if (decodedPayload) {
            const processingTime = Date.now() - startProcessing;
            // Add raw hex for storage
            decodedPayload.rawHex = payload.toString('hex').toUpperCase();
            // Store in database
            storeUC300OfficialData(deviceId, topic, decodedPayload, processingTime);
            // Log every 100th message for monitoring
            if (messageCount % 100 === 0) {
                const readableStatus = official_uc300_decoder_1.OfficialUC300Decoder.getReadableStatus(decodedPayload);
...
async function main() {
    try {
        ...
        const client = mqtt_1.default.connect(mqttConfig);
        client.on('connect', () => {
            ...
            const commandSender = new uc300_command_sender_1.UC300CommandSender(client, '6445E28148720016');
