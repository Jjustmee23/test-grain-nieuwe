...
export class OfficialUC300Decoder {
   ...
   */
  private static decodeChangeReport(buffer: Buffer, offset: number): UC300ChangeReport | null {
    try {
      // Packet Version
   ...
   */
  private static decodeAttributeReport(buffer: Buffer, offset: number): UC300AttributeReport | null {
    try {
      // Packet Version
      const version = buffer.readUInt8(offset++);
   ...
   */
  private static decodeRegularReport(buffer: Buffer, offset: number): UC300RegularReport | null {
    try {
      // Packet Version
      const version = buffer.readUInt8(offset++);
      ...
      while (offset < buffer.length - 1) { // -1 for end flag
        if (offset + 2 > buffer.length - 1) break;
  ...
  static getReadableStatus(payload: UC300Payload): any {
    ...
    const result: any = {
      dataType: dataTypeNames[payload.dataType as keyof typeof dataTypeNames] || `Unknown (0x${payload.dataType.toString(16).toUpperCase()})`,
      packetLength: payload.packetLength
    };
    
    if (payload.dataType === 0xF2) {
      const changeData = payload.data as UC300ChangeReport;
    ...
    if (payload.dataType === 0xF3) {
      const attrData = payload.data as UC300AttributeReport;
      result.deviceInfo = {
    ...
    if (payload.dataType === 0xF4) {
      const regularData = payload.data as UC300RegularReport;
