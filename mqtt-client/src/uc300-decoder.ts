...
export class UC300Decoder {
  ...
  static decodeStatusPayload(deviceId: string, payload: Buffer): UC300Payload | null {
    try {
      ...
      if (payload.length >= 8) {
        ...
        if (analogs.length > 0) {
          if (!decoded.decodedData) decoded.decodedData = {};
   ...
   */
  private static findDeviceIdInPayload(payload: Buffer, expectedDeviceId: string): string | null {
   ...
   */
  private static extractCounters(payload: Buffer): number[] {
   ...
   */
  private static extractAnalogValues(payload: Buffer): number[] {
   ...
   */
  private static extractDigitalStates(payload: Buffer): boolean[] {
  ...
  static analyzePayload(payload: Buffer): any {
    ...
    for (let i = 0; i < payload.length; i++) {
      const byte = payload.readUInt8(i);
      if (byte === 0) analysis.patterns.zeros++;
      if (byte === 1) analysis.patterns.ones++;
      if (i > 0 && byte === payload.readUInt8(i - 1)) analysis.patterns.repeating++;
    }
